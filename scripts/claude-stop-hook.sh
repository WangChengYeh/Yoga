...
RATE_LIMIT_WATCHDOG="$(cd "$(dirname "$0")" && pwd)/claude_rate_limit_watchdog.sh"
...
# Best-effort rate-limit watchdog check.
if [ -x "$RATE_LIMIT_WATCHDOG" ]; then
  "$RATE_LIMIT_WATCHDOG" --once >/dev/null 2>&1 || true
fi
