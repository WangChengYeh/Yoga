...
for agent_file in "$PROJECT_ROOT/AGENTS.md" "$PROJECT_ROOT/CLAUDE.md" "$PROJECT_ROOT/GEMINI.md"; do
  [ -f "$agent_file" ] || continue
  agent_name="$(sed -n '1s/^# \(.*\) Agent$/\1/p' "$agent_file" | tr '[:upper:]' '[:lower:]')"
  [ -n "$agent_name" ] || continue
  [ "$agent_name" = "$SELF_AGENT" ] && continue
  echo "[$TS] Hello, ${agent_name}! Gemini here." >> "$LOG_FILE"
done
