...
object PoseDetectionRouter {
    ...
    fun evaluate(
        ...
        return when (poseId) {
            "forward_fold" -> {
                val r = ForwardFoldDetectionMapper.evaluate(detect, frame)
            ...
            "squat" -> {
                val r = SquatDetectionMapper.evaluate(detect, frame)
                PoseDetectionMappingResult(r.matched, r.state, r.cue)
            }
            "bridge" -> {
                val r = BridgeDetectionMapper.evaluate(detect, frame)
                PoseDetectionMappingResult(r.matched, r.state, r.cue)
            }
            else -> {
