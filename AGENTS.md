...
This project uses **CLI_Cowork_Bridge** (WangChengYeh/CLI_Cowork_Bridge) as the multi-agent workspace.
...
## Role
- Primary implementer for YogaFlow 3D PM workflow (`writer` role in CLI_Cowork_Bridge)
- Handles deep code changes, multi-file refactors, Gradle builds, `adb` device testing, and PR-ready commits

## Invocation
- Via CLI_Cowork_Bridge: Claude PM types `/ask writer <task>` inside the CLI_Cowork_Bridge session
...
**Godot avatar layout:** `virtualCoachView` (GodotFragment) is a **full-screen transparent overlay** stacked on top of the camera preview and pose skeleton. The Android view itself never moves; the avatar Node3D inside Godot tweens between positions. Avatar positioning has three precedence layers (highest first): `override_position {x, y}` raw world coords (latched) → `position` semantic shortcut name (`left_side`, `right_side`, `center`, `demo_area`, `near_knees`, `near_hips`, `near_spine`) → `demo_area` default. See `docs/avatar.md` §3, §5 for details. ADB free-move extras: `--ef avatarTargetX -1.5 --ef avatarTargetY 0.0` (then `--ez avatarClearOverride true` to release).
