...
## Agent Architecture: CLI_Cowork_Bridge + Role Definitions
...
cd cli_cowork_bridge && ./install.sh install && ccb update
```
...
ccb kill   # stop CLI_Cowork_Bridge runtime
ccb -n     # rebuild runtime, keep config
```

### Primary delegation workflow (CLI_Cowork_Bridge)
Claude PM writes a structured task prompt and delegates via CLI_Cowork_Bridge:
```
...
### Fallback invocation (CLI_Cowork_Bridge unavailable)
When CLI_Cowork_Bridge is not running, fall back to direct tool invocation:
